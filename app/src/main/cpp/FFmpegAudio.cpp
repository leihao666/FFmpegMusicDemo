...
void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    ...
    if (datalen > 0) {
        ...
        LOGI("数据长度%d  分母%d  值%f 通道数%d", datalen, 44100 * 2 * 2, time, audio->out_channel_nb);
        audio->clock += time;
        LOGI("当前一帧声音时间%f   播放时间%f", time, audio->clock);
        (*bq)->Enqueue(bq, audio->out_buffer, datalen);
        LOGI("播放 %d ", audio->queue.size());
    } else
        LOGE("解码错误");
}
...
int createFFmpeg(FFmpegAudio *audio) {
    ...
    //设置转换上下文
    swr_alloc_set_opts(audio->swrContext, out_ch_layout, out_sample_fmt, out_sample_rate,
    ...
    LOGI("------>通道数%d  ", audio->out_channel_nb);
    return 0;
}
