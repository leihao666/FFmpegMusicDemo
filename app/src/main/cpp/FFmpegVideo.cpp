...
void *play_video(void *arg) {
    ...
    LOGI("宽  %d,  高  %d  ", video->codec->width, video->codec->height);
    //设置rgbFrame的缓冲区的像素格式
    ...
    while (video->isPlay) {
        LOGI("视频 解码  一帧 %d", video->queue.size());
        //消费者取到一帧数据  没有 阻塞
        video->get(packet);
        ...
        LOGI("输出%d ,%d", code, len);
        LOGI("宽%d  高%d  行字节 %d  状态%d ====%d", frame->width, frame->height, rgb_frame->linesize[0],
             got_frame, video->codec->height);
